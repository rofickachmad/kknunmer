...
class fakultas extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('fakultas_model','fakultasdb',TRUE);
        $this->session->set_userdata('lihat','fakultas');
    ...
    public function index() {
        $this->template->set_title('Kelola Fakultas');
        $this->template->add_js('var baseurl="'.base_url().'fakultas/";','embed');  
        $this->template->load_view('fakultas_view',array(
            'view'=>'',
            'title'=>'Kelola Data Fakultas',
            'subtitle'=>'Pengelolaan Fakultas',
            'breadcrumb'=>array(
            'Fakultas'),
        ));
    }
    ...
    public function getdatatables(){
            ...
            $this->datatables->select('kodef,{fields_tabel1}{name_field_table},{/fields_tabel1}')
                            ->from('{nama_tabel}');
            ...
            $this->datatables->unset_column('kodef');
    ...
    public function get($kodef=null){
        if($kodef!==null){
            echo json_encode($this->{nama_tabel}db->get_one($kodef));
        }
    }
    function tables(){
    ...
    public function submit(){
        if($this->__formvalidation()===TRUE):
            ...
            if ($this->input->post('ajax')){
              if ($this->input->post('kodef')){
                $this->{nama_tabel}db->update($this->input->post('kodef'));
              }else{
    ...
    public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['kodef'])){
                $this->{nama_tabel}db->upddel_detail($this->input->post('kodef'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
     ...
     public function delete_detailxx(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['kodef'])){
                $this->{nama_tabel}db->delete_detail($this->input->post('kodef'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
