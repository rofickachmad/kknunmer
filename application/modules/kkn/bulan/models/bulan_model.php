...
class Bulan_model extends CI_Model {
    ...
    function get_all($limit, $uri) {
        ...
        $result = $this->db->get('bulan', $limit, $uri);
    ...
    function upddel_detail($bulan=null) {
    ...
    function updatebyid($bulan,$data){
        $this->db->where('bulan', $bulan);
        $this->db->update('{nama_tabel}', $data);
    }
    function update($bulan) {
        $data = array(
        'bulan' => $this->input->post('bulan',TRUE),{fields_tabel2}
       '{name_field_table}' => $this->input->post('{name_field_table}', TRUE),
       {/fields_tabel2}
        );
        $this->db->where('bulan', $bulan);
    ...
    function get_dropdown_array($value){
        $result = array();
        $array_keys_values = $this->db->query('select bulan, '.$value.' from {nama_tabel} order by bulan asc');
        $result[0]="-- Pilih Urutan bulan --";
        foreach ($array_keys_values->result() as $row)
        {
            $result[$row->bulan]= $row->$value;
        }
        return $result;
    }
