 ...
 * Update 11/11/2018 roniwahyu@gmail.com
 * Compatibility with Themes and Layout
 */
...
class Template {
    ...
    protected $brand_name = 'Universitas Merdeka Malang'; 

    protected $title_separator = ' | ';
    protected $ga_id = FALSE; // UA-XXXXX-X

    protected $layout = 'default';
    // protected $theme = 'coolui';
    protected $theme = '';
    public $themebase = '';
    ...
    {
        if(!empty($this->themebase)):
            
            $this->baseview = $this->themebase.$baseview;
            // $this->themebase="themes/".$theme."/";
        // else:
            // $this->baseview=$this->$baseview;
        endif;
        // print_r($this->baseview);
    }
    /**
     * updated by Syahroni Wahyu - roniwahyu@gmail.com - 04042014
    ...
    /**
     * Menambahkan Assets berbagai type js,css,atau images
     *
     * @access  public
     * @param   string  $js
     * @return  void
     */
    public function add_assets($src,$type=FALSE)
    {
        // $this->_ci->load->helper('url');
        // $this->js['js']=$src;
        $this->assets[]=array('src'=>$src,'type'=>$type);
    ...
    public function load_view($view, $data = array(), $return = FALSE){
        ...
        */
        ...
        /*
            Increase flexibility using assets link
            updated by Syahroni Wahyu - roniwahyu@gmail.com - 11/11/2015
        */
        $assets = array();
        foreach ($this->assets as $assets_item){
            $type=$assets_item['type'];
            $src=$assets_item['src'];
            
            $filepath =assets_url($src);
            switch ($type){
                 case 'js':
                    $src = '<script type="javascript" src="'. $filepath .'"><script>';
                    $assets[]=$src;
                    break;
                case 'css':
                    $src = '<link type="text/css" rel="stylesheet" href="'. $filepath .'"/>';
                    $assets[]=$src;
                    break;
        ...
        if(!empty($this->themebase)||$this->themebase!=''):
            $header = $this->_ci->load->view($this->themebase.'header', array(), TRUE);
            $footer = $this->_ci->load->view($this->themebase.'footer', array(), TRUE);
        else:
            // print_r($this->themebase);
        ...
        // print_r($view);
        $body = $this->_ci->load->view($this->themebase.'layout/' . $this->layout, array(
            'header' => $header,
            'footer' => $footer,
    ...
    function header($header){
            ...
            $html= $this->_ci->load->view($header, array(), TRUE);
            return $html;
        }
    function footer($footer){

            $html= $this->_ci->load->view($footer, array(), TRUE);
            return $html;
        }
